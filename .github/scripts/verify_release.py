...
"""Verify promoted ASF source files before GitHub or Maven publication."""
...
def verify(version, output):
    ...
    url = ASF + 'jcasbin-' + version + '-incubating/'
    with tempfile.TemporaryDirectory(prefix='jcasbin-public-keys-') as temp:
        home = Path(temp)
        home.chmod(0o700)
        keys = home / 'KEYS'
