...
class ReleaseVerificationTest(unittest.TestCase):
    def setUp(self):
        ...
        self.git('init', '-q')
        Path('pom.xml').write_text('<project xmlns="http://maven.apache.org/POM/4.0.0"><version>1.0.0</version></project>')
        ...
        self.name = 'apache-casbin-jcasbin-1.0.0-incubating-src'
    ...
    def put_archive(self, extension, data):
        ...
        url = release.ASF + 'jcasbin-1.0.0-incubating/' + filename
    ...
    def test_preserves_all_six_promoted_files(self):
        self.invoke()
        files = list(Path('dist').iterdir())
        self.assertEqual(len(files), 6)
        for file in files:
            self.assertEqual(file.read_bytes(), self.remote[release.ASF + 'jcasbin-1.0.0-incubating/' + file.name])

    def test_rejects_missing_signature(self):
        self.remote.pop(release.ASF + 'jcasbin-1.0.0-incubating/' + self.name + '.tar.gz.asc')
    ...
    def test_rejects_corrupt_archive(self):
        self.remote[release.ASF + 'jcasbin-1.0.0-incubating/' + self.name + '.tar.gz'] += b'corrupt'
    ...
    def test_rejects_zip_missing_file_with_valid_checksum(self):
        buf = io.BytesIO()
        with zipfile.ZipFile(buf, 'w') as archive:
            archive.writestr(self.name + '/pom.xml', Path('pom.xml').read_bytes())
        self.put_archive('zip', buf.getvalue())
        with self.assertRaises(ValueError):
            self.invoke()
